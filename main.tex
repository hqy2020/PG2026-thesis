...
表~\ref{tab:ablation} 采用更接近顶会常见写法的“单模块剥离 + 渐进构建”结构汇报 3-view 消融结果。可以看到，\gap\ 单独使用就带来了最明显的收益，说明结构冗余控制是当前任务中的主导因素；\sps\ 与 \adm\ 则分别从初始化与空间密度修正两个角度提供补充，因此完整模型保留三者联合。
...
我们将最终采用的关键超参数整理在表~\ref{tab:hyperparams} 中。整体来看，\sps\ 与 \adm\ 的可行区间相对较宽，而 \gap\ 的阈值与最大剪枝比更敏感，因此我们优先固定其余模块，再围绕冗余控制强度做小范围调参。
