...
## 机制类 figure 默认模板（2026-05-28 落定）

**机制类 figure**（pipeline / 单模块示意图，如 method_fig_pipeline / sps / gap / adm）默认走「单行 1 行 × 3 列横向三段式」，**不**用 2×N 网格（2×N 留给比较类）：

- 整体宽高比约 **16:5**（约 1600 × 500 px，dpi ≥ 300），白底
- **列分工固定**：左 = 输入 / 前态，中 = 模块机制 / 判据 / 公式，右 = 输出 / 后态
- 列间用 1.5pt 灰色 `#888888` 实线右向箭头连接，箭头中段标 6pt 灰色短词（如 `KNN + gradient` / `bilinear sample`）
- 顶部居中加粗无衬线列标题（8–9pt），三列等宽等高
- 列标题用功能性短语（如 `After Densification` / `Joint Criterion` / `After Pruning`），不用 `(a)(b)(c)` panel 编号
- 同组 4 张机制图必须**几何同源**（如全部用同一头骨灰色线稿轮廓），椭球比例、箭头粗细、字号严格一致
- 模块主色映射：SPS 蓝 `#1F77B4` / GAP 橙 `#FF7F0E` / ADM 绿 `#2CA02C`；pipeline 图用全部三色 badge
- SPS / GAP / ADM badge **只在 pipeline 图出现一次**，单模块图不再加 badge（避免视觉冗余）
- 30 秒可读性目标：不读正文的人能复述「左输入 → 中机制 → 右输出」的核心 dichotomy
